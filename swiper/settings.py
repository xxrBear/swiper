...
SECRET_KEY = 'v+k759wox^c%9trvk4(j)ee(zga@eb_$tjnuz8a(z!+e%cqtct'
...
LOGGING = {
    'version': 1,
    'disable_existing_loggers': True,  # 格式配置
    'formatters': {
        'simple': {
        ...
        'verbose': {
            'format': ('%(asctime)s %(levelname)s [%(process)d-%(threadName)s] ''%(module)s.%(funcName)s line %(lineno)d: %(message)s'),'datefmt': '%Y-%m-%d %H:%M:%S',
        }
    },
    # Handler 配置
    'handlers': {
        ...
        'error': {
            'class':
                'logging.handlers.TimedRotatingFileHandler',
            'filename':
                f'{BASE_DIR}/logs/error.log',  # ⽇志保存路径
            'when': 'W0',  # 每周⼀切割⽇志
